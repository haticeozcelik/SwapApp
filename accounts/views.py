...
from django.contrib import messages  # Django framework'ü içinde kullanılan mesajlaşma sistemi
from django.contrib.auth import authenticate, login, logout  # Kullanıcı doğrulama, giriş ve çıkış işlemleri
from django.contrib.auth.models import User  # Django'nun varsayılan kullanıcı modeli
from django.core.files.storage import FileSystemStorage  # Dosya yükleme ve saklama işlemleri
from django.shortcuts import redirect, render, get_object_or_404  # Yönlendirme ve şablon render etme işlemleri
from django.core.mail import send_mail  # E-posta gönderme fonksiyonu
from django.contrib.sites.shortcuts import get_current_site  # Django sitesine ait bilgileri almak için
from django.template.loader import render_to_string  # Şablonları string olarak render etmek
from django.utils.http import urlsafe_base64_encode, urlsafe_base64_decode  # URL'leri güvenli bir şekilde encode ve decode etmek
from django.utils.encoding import force_bytes, force_str  # Byte ve string dönüşümleri
from django.contrib.auth.tokens import default_token_generator  # Django'nun varsayılan token oluşturucusu
from django.http import HttpResponse  # HttpResponse sınıfı (genellikle özel HTTP cevapları oluşturmak için)
from swapapp import settings  # settings dosyası (proje ayarları)
from accounts.models import *  # accounts uygulamasındaki tüm modeller
...
def rate_course(request):
    ...
    given_rating = request.POST['givenRating']
    course = Course.objects.get(name=course_name)

    current_rating = course.rating
    current_count = course.rating_count

    student_rating = float(given_rating)

    new_rating = ((current_rating * current_count) + student_rating) / (current_count + 1)
    new_rating = new_rating.__round__(2)

    current_count += 1

    course.rating = new_rating
    course.rating_count = current_count
...
def register_instructor_view(request):
    if request.method == "POST":
        ...
        if password == repassword:
            ...
            elif not email.endswith('@ogr.alanya.edu.tr'):  # emaili edu olanlar girmesin
